...
while True:
    try:
        ...
        # Option 3 - Format partitions or disks
            ...
            console.print("[yellow]⚠️ Warning: if it's removable media, plug it now.[/yellow]")
        ...
        # Option 4 - Heimdall
            ...
            if dq1 == 1:
                ...
                confirm = Prompt.ask(f"⚠️ Are you sure you want to flash {dq2} with {dq3}? (y/n)")
                if confirm.lower() == "y":
        ...
        # Option 7 - Remove files or folders (rm)
            ...
            if os.path.isdir(hq1):
                cmd = ["rm", "-r", hq1] if EF == 1 else ["rm", "-rf", hq1]
            else:
                cmd = ["rm", hq1] if EF == 1 else ["rm", "-f", hq1]

            if ES == 2:
                cmd.insert(0, "sudo")

            confirm = Prompt.ask(f"⚠️ Are you sure you want to delete {hq1}? (y/n)")
            if confirm.lower() == "y":
                subprocess.run(cmd)
